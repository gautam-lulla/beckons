/**
 * Upload compressed large images
 */
...
const IMAGES = [
  {
    filename: "about-card-image-1-compressed.jpg",
    mimeType: "image/jpeg",
    alt: "Places Worth Reaching - lodge building",
  },
  {
    filename: "lodge-carousel-bg-compressed.jpg",
    mimeType: "image/jpeg",
...
async function uploadImage(
  ...
  if (result.errors) {
    console.error(`  Errors:`, result.errors[0]?.message);
    return null;
  }
...
async function main() {
  ...
  for (const image of IMAGES) {
    ...
    const url = await uploadImage(image.filename, image.mimeType, image.alt, authToken);
    if (url) {
      console.log(`  ✓ ${url}\n`);
    } else {
      console.log(`  ✗ Failed\n`);
